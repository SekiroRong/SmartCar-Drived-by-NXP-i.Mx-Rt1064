...
uint16 time1,time2,tick;
float divertion,velocity,velocity_angle;
int16 ftmave,ideal_velocity;
int16 FTM_ave=0;
uint8 tick_mark=0;
...
void PIT_IRQHandler(void)
{
    ...
    {
      //count++;
     //divertion=Diversion_PD();
     //Div_output_filter(divertion);
     //FTM_PID((vall-valr)/2,15);
    //freql=FTM_PID(valr,50);
      if(tick_mark!=0)
        tick++;
      else
        tick=0;
    ...
    if(time2==20)
    {
      velocity=FTM_PI(FTM_ave,ideal_velocity);
      time2=0;
    }
    //if(chuku==1)
   // {
